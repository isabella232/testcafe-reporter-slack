...
var IncomingWebhook = _require.IncomingWebhook;

var SlackMessage = (function () {
  function SlackMessage() {
    _classCallCheck(this, SlackMessage);

    this.slack = new IncomingWebhook(_config2["default"].webhookUrl);
  ...
  _createClass(SlackMessage, [{
    ...
    value: function sendMessage(message) {
      ...
      var formattedMessage = typeof message === "string" ? this.convertTextToBlock(message) : message;

      this.slack.send(Object.assign(_extends({
        channel: _config2["default"].channel,
        username: _config2["default"].username
      }, formattedMessage), slackProperties)).then(function (response) {
        if (!_config2["default"].quietMode) {
          console.log("The following message is send to slack: \n " + JSON.stringify(formattedMessage, undefined, 2));
        }
      })["catch"](function (err) {
